...
public class Empleado_Caso {
    ...
    private void ByeEmpleadoCaso() throws SQLException{
     ...
     public void Asignar(int id_programador, int id_tester) throws SQLException, ClassNotFoundException {
        String sql = "insert into empleados_caso  values(?,?) ";
    ...
    public void ModificarEmpleados(int id_programador, int id_tester, String nom_p, String nom_t, String id) throws SQLException, ClassNotFoundException {
        ...
        String sql = "update empleados_caso ec INNER JOIN empleados emp ON  ec.id_empleado=emp.id_empleado set ec.id_empleado=? WHERE  CONCAT(emp.nombre_emp,' ',emp.apellidos)=? and ec.id_caso=?";
    ...
    public Boolean verificarAsignacion(String id) throws SQLException {   //Si ya estan los dos empleados asignados no se insetaran mas solo se modificaran
        Boolean band = false;
        String sql = "SELECT count(id_caso) from empleados_caso where id_caso='" + id + "'";
    ...
    public String nombreEmpleadoP(String id) throws SQLException {
        String nombre;
        String sql = "Select concat(nombre_emp,' ',apellidos) from empleados emp Inner Join empleados_caso ec ON ec.id_empleado=emp.id_empleado where id_cargo=3 and"
                + " ec.id_caso='" + id + "'";
        ResultSet datos = con.Buscar(sql);
        datos.next();
    ...
    public int idTester(String id) throws SQLException {
        int cod;
        String sql = "Select ec.id_empleado from empleados emp Inner Join empleados_caso ec ON ec.id_empleado=emp.id_empleado where id_cargo=4 and"
                + " ec.id_caso='" + id + "'";
        ResultSet datos = con.Buscar(sql);
        datos.next();
        //JOptionPane.showMessageDialog(null, datos.getInt(1));
    ...
    public int idProgramador(String id) throws SQLException {
        int cod;
        String sql = "Select ec.id_empleado from empleados emp Inner Join empleados_caso ec ON ec.id_empleado=emp.id_empleado where id_cargo=3 and"
                + " ec.id_caso='" + id + "'";
        ResultSet datos = con.Buscar(sql);
        datos.next();
        // JOptionPane.showMessageDialog(null, datos.getInt(1));
